...
A — CURRENT_REPO_VERIFIED: CRM Activity Timeline Performance V1 (#1), Weekly Business Single-Week SQL Boundary V1 (#3), and Opportunity Detail Selective RAW Hydration V1 (#2) are implemented. Their implementation sequence is #1, #3, then #2; the original engineering ranking remains unchanged. Targeted static/unit validation is code-level evidence, not a runtime benchmark or Production Performance PASS. Candidates #4 and #5 remain future work.
...
The descriptions below distinguish A — CURRENT_REPO_VERIFIED completed/current paths from D — NEXT_WORKSTREAM_CONSTRAINT / RECOMMENDATION future directions and preservation requirements.

1. **Interactions / CRM Activity Timeline.** Current: `/api/activity-timeline` → `getActivityTimeline()` → `getActivityTimelineInteractions()`, owned by `services/activity-timeline-service.js`, `services/interaction-service.js`, and `data/interaction-sql-reader.js`. Performance V1 reads only Timeline-required interaction facts, preserves classification/filter/audit merge/order/final pagination, then batch-hydrates related names for visible interaction rows. It no longer invokes the general full opportunity/company enrichment path. Audit-fetch/count redesign and true merged-source server pagination remain separate.
2. **Opportunities / Opportunity Detail.** Current: `/api/opportunities/:id/details` → `getOpportunityDetails()` → capture-ordered lightweight RAW identity/company discovery → existing normalized-company selection → one `getRawContactsByCardIds()` hydration for the deduplicated union of same-company and linked-source cards. The Detail path no longer calls `getRawContacts()` or performs N+1 RAW hydration. Existing linked/CORE enrichment, RAW potential merge, and same-person deduplication remain. Broad company lookup remains intentionally unchanged; company fuzzy reconciliation is not part of this slice.
3. **Dashboard / Weekly Business.** Current: Dashboard → `getWeeklyDetails(weekId)` → `getEntriesForWeek(weekId)` → `getWeeklyBusinessEntriesByWeekId(weekId)`, owned by `services/dashboard-service.js`, `services/weekly-business-service.js`, and `data/weekly-business-sql-reader.js`. Performance V1 applies `week_id = requested weekId` on successful SQL reads, then preserves existing normalization/filter/sort/day projection. A scoped SQL failure still invokes the broad Sheet read and service week filter; successful empty SQL results remain authoritative. Multi-week summary paths remain broad, Weekly Sheet fallback remains `COMPATIBILITY_REQUIRED`, and Google Calendar remains `ACTIVE_OR_PROTECTED`.
