...
A — CURRENT_REPO_VERIFIED: CRM Activity Timeline Performance V1 (#1) is implemented with a consumer-specific narrow interaction projection and final-page related-name hydration. Targeted static/unit validation is code-level evidence, not a runtime benchmark or Production Performance PASS. Candidates #2–#5 remain future work. The original ranking below remains unchanged and is not an implementation sequence.
