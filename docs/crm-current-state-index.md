...
* A — CURRENT_REPO_VERIFIED: **Opportunity Detail Selective RAW Hydration V1** now uses a capture-ordered lightweight RAW identity/company index, existing application normalization, and one deduplicated `getRawContactsByCardIds()` hydration for same-company and linked-source cards. The broad company read remains. This is code-level evidence only, not a runtime or Production Performance PASS.
...
* Performance boundary: `getOpportunityDetails()` no longer calls `getRawContacts()` to hydrate the complete RAW pool. Same-company discovery uses a lightweight RAW projection and existing normalization before one selective card-ID batch hydration. Current broad company lookup remains intentionally unchanged.
